...
> **参考项目（开源）**：https://github.com/CJ-tpub/vs2019-sticky-scroll —— VS2019 粘滞滚动扩展，
> 含完整实现、离线构建脚本与打包流程，可对照本技能学习（注意：项目自身细节请勿照搬，以本技能的通用经验为准）。
...
# 仅当 HTTPS 直连不通、且本机配置了代理时才需要设置（<代理地址> 形如 http://proxy.example.com:8080）；无代理环境跳过这两行
