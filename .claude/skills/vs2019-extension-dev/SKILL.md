...
description: Visual Studio 2019 VSIX 编辑器扩展开发的完整实战指南——开发流程、关键 API、离线构建、VSIX 打包、安装部署、调试方法、测试与发布流程，以及本机/工具链相关的全部已知坑与解决方案。当任务涉及 VS2019 扩展、VSIX 打包、IWpfTextViewMargin/adornment、MEF 编辑器组件、粘滞滚动类功能时使用。
---

# VS2019 扩展开发实战指南（VSIX / 编辑器扩展）

> 本技能沉淀自"StickyScroll for VS2019"扩展的完整开发过程（仓库：CJ-tpub/vs2019-sticky-scroll），
> 覆盖从零到发布的全流程与踩坑记录。**先读"环境前提"，再按"开发流程"执行，遇到问题查"避坑清单"。**
...
| .NET Framework targeting pack | 检查 `C:\Program Files (x86)\Reference Assemblies\Microsoft\Framework\.NETFramework\v4.7.2` | 缺失时用 setup-ref.ps1 离线生成（见 B3） |
| HTTPS 外网（NuGet/GitHub） | `curl -I https://www.baidu.com` | 不通 → 走离线构建路线（B3）；git 可配代理（HTTP_PROXY） |
| VSSDK BuildTools | `%VS2019路径%\MSBuild\Microsoft\VisualStudio\v16.0\VSSDK` | 无 → 手工打包 vsix（见 C） |
...
- `build.ps1`：MSBuild 编译 + 手工 OPC 打包（见 C）
- **先做最小冒烟**：只放一个空 margin（固定文本），验证「编译→打包→安装→VS 显示」全链路，再开发功能

### 2.3 迭代开发循环
```
改代码 → build.ps1 → 拷贝 DLL 到扩展目录（需先授权 ACL，见 D1）→ 重启 VS → 验证 → 改
```
...
2. manifest 的 Asset 用 `d:Source="File" Path="StickyScroll.dll"`（手工打包不能写 d:Source="Project"）。
...
| 检测语法块 | `IOutliningManager.GetAllRegions(snapshot)`（注意签名是 SnapshotSpan 参数；**C# 的 outlining 只有声明级区域**：namespace/class/method，**没有 if/for/while 等语句块**）→ 语句块需自写扫描器（花括号状态机，忽略注释/字符串）+ 白名单过滤（排除含 `=`/`=>`/`new` 的初始化/lambda 行） |
...
| 点击跳转 | `IViewScroller.ScrollViewportVerticallyByPixels(delta)`——**实测符号：正值 = 内容下移（往前滚）**，delta = `ViewportTop - targetY`；目标行 Y = 行号 × 当前缩放行高（`FirstVisibleLine.Height`，不能用 `_view.LineHeight` 未缩放值） |

### 4.1 几何对齐（粘滞栏与编辑器逐像素对齐）
- **坐标体系**：top margin 的 x=0 ≠ viewport 左缘——二者之间隔着 outlining/行号/selection 等左 margin。用 WPF `TranslatePoint` 实测：
  ...
  - 文本列（margin 坐标）= viewport 原点 + `FirstVisibleLine.TextLeft`（+1.5px 渲染微调）
  - 行号数字右缘：**行号 margin 元素右缘** `margin.VisualElement.TranslatePoint((ActualWidth,0), _root).X`——**VS 行号内容经缩放变换放大**（元素 39px × zoom），TranslatePoint 自动计入；这是唯一与像素实测吻合的方案
- **DPI 陷阱**：PowerShell 默认非 DPI-aware，`GetWindowRect` 返回虚拟坐标导致像素测量全乱 → 先 `SetProcessDPIAware()`
- **像素验证**：`CopyFromScreen` 全屏截图 + `GetPixel` 分析（**PrintWindow 对 GPU 渲染内容颜色会失真**，不要用）；截图的浅蓝/浅色判定会误匹配浏览器等区域，扫描范围要限定在 VS 窗口区域

### 4.2 性能
- 粘滞链计算缓存：按 `ITextSnapshot` 缓存扫描结果；滚动时链相同跳过重绘（SameChain + 几何度量变化检测）
- 缩放/几何变化必须触发强制重绘（检测 ZoomLevel/文本列/行号列宽/行号右缘的变化）

## 五、测试流程（发布前逐项回归）

1. 滚动链正确性：文件中部显示 namespace→class→method 链；**文件顶部自动隐藏**
2. **语句块**：if/for/while/try/else 块内对应行粘滞；`} else {` 行也粘滞
3. **误判规避**：注释/字符串中的 `{` 不粘滞；字典初始化/`new X {`/lambda 不粘滞；.txt 文件不显示
4. **点击跳转**：各缩放（100%/150%/200%/300%）点击粘滞行 → 目标行落在粘滞栏下方第一行
5. **缩放跟随**：Ctrl+滚轮缩放，字体/行高/缩进/行号列与代码同步
6. **编辑实时性**：增删行、改内容 → 粘滞链立即更新
7. **主题切换**：浅色/深色背景色正确
8. **性能**：万行大文件滚动流畅
9. **卸载干净**：VSIXInstaller /uninstall 后无残留
10. 与折叠、word wrap、缩放共存正常

## 六、安装 / 部署 / 权限

- 安装：双击 vsix（走 VSLauncher 向导）或 `VSIXInstaller.exe /q /admin xxx.vsix`（**需要管理员/UAC**——写注册表 hive）
- 升级：双击更高版本 vsix 覆盖即可；同版本提示"已安装"→ 先卸载或 `/force`
- 卸载：`VSIXInstaller.exe /q /uninstall:<Id>`
- 实验实例：`VSIXInstaller /rootSuffix Exp xxx.vsix`（开发调试不污染正式环境）
...
gh release create vX.Y.Z 'StickyScroll.vsix' --title '...' --notes '...'
gh release upload vX.Y.Z 'file.vsix' --clobber   # 更新附件
```
...
- **README 中的操作步骤必须实测确认**（如"从文件安装扩展"按钮是 VS2022 才有，VS2019 没有——写错会被用户指出）
...
- 粘滞行检测的深度：outlining 深度 ≠ 括号深度，两条链合并时深度要统一处理（outlining 独有行深度按"下一个扫描候选深度"推断）
...
- 编译器用 csc 3.11（C# 8）：`LangVersion=latest`；注意 .NET 4.0/4.7.2 目标下 ValueTuple 等需谨慎（4.7.2 内置）
